...
def is_done(existing_entry):
    ...
    current model and meets the depth floor -- old Claude-authored content or a
    prior thin attempt does not count, and gets regenerated."""
    if not existing_entry:
        return False
    if existing_entry.get("source") != SOURCE_TAG:
        return False
    return (len(existing_entry.get("phases", [])) >= MIN_PHASES and
